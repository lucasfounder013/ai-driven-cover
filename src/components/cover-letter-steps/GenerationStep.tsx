...
export const GenerationStep = ({
  ...
  const fetchProfile = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from("profiles").select("first_name, last_name, phone_number, professional_email, linkedin_url").eq("id", user.id).single();
    if (error) console.error("Error fetching profile:", error);else setProfileData(data);
  };
  ...
  const generateLetter = async () => {
    ...
    try {
      ...
      const {
        data: cvData,
        error: downloadError
      } = await supabase.storage.from("cvs").download(cvPath);
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke("generate-cover-letter", {
        body: {
          jobTitle,
          companyName,
          jobDescription,
  ...
  const downloadLetter = async () => {
    try {
      ...
      const name = profileData ? `${profileData.first_name?.toUpperCase() || ""} ${profileData.last_name?.toUpperCase() || ""}` : "NOM PRÉNOM";
      const subtitle = "Stage de 6 mois à partir de Février 2026";
      const contact = [profileData?.phone_number || "", profileData?.professional_email || "", profileData?.linkedin_url || ""].filter(Boolean).join("  •  ");
      ...
      doc.text(contact, pageWidth / 2, topMargin, {
        align: "center"
      });
      doc.setFont("Helvetica", "bold");
      doc.setFontSize(18);
      doc.setTextColor(0, 0, 0);
      doc.text(name, pageWidth / 2, topMargin + 10, {
        align: "center"
      });
      doc.setFont("Helvetica", "normal");
      doc.setFontSize(12);
      doc.setTextColor(100, 100, 100);
      doc.text(subtitle, pageWidth / 2, topMargin + 17, {
        align: "center"
      });
      doc.setDrawColor(0);
      ...
      doc.setFontSize(13);
      doc.setTextColor(0, 0, 0);
      doc.text(title, pageWidth / 2, topMargin + 32, {
        align: "center"
      });
  ...
  if (!generatedLetter) {
    return <div className="flex flex-col items-center justify-center py-16 space-y-6 max-w-3xl mx-auto">
        ...
        <div className="w-full space-y-2">
          ...
          <Textarea id="additionalInfo" placeholder="Ajoutez ici toute information pertinente qui ne figure pas sur votre CV (projets spécifiques, motivations particulières, compétences à mettre en avant, etc.)" value={additionalInfo} onChange={e => setAdditionalInfo(e.target.value)} className="min-h-[120px]" />
  ...
  return <div className="space-y-6">
      ...
      <Textarea value={generatedLetter} onChange={e => setGeneratedLetter(e.target.value)} className="min-h-[400px] font-serif" />
